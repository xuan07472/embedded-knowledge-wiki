...
* 进行一个新项目时，如果是裸机，先想好文件夹结构和其中的各个模块，如：  
...
* 如果使用芯片厂商的开发包，则可以使用Demo中的框架。
* 如果使用操作系统，则可以只管理好应用的文件夹结构，驱动按照操作系统的规定按需填充裁剪。
...
1. 显示功能需要哪些硬件条件：
  ...
  * 包含GPU的嵌入式芯片有：全志、瑞芯微、海思等，这和低端嵌入式产品被ST、TI、NXP等国外厂商占据大部分份额不同，国内厂商更受欢迎。
